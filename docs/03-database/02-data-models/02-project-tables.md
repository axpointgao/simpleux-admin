...
| manager_id        | uuid          | FK → profiles.id, NOT NULL         | 项目经理 ID                              |
...
| framework_id      | uuid          | FK → framework_agreements.id, NULL | 计件项目 ID                              |
...
| supplier_id   | uuid          | FK → suppliers.id, NULL    | 供应商 ID  |
...
| id              | uuid          | PK                         | 支出 ID                           |
| project_id      | uuid          | FK → projects.id, NOT NULL | 项目 ID                           |
| employee_id     | uuid          | FK → profiles.id, NOT NULL | 员工 ID                           |
...
| attachment_url     | text          | NULL                       | 变更附件 URL               |
| approval_id        | uuid          | FK → approvals.id, NULL    | 关联的审批 ID              |
| created_by         | uuid          | FK → profiles.id, NOT NULL | 创建人 ID                  |
| created_at         | timestamp     | NOT NULL, DEFAULT now()    | 创建时间                   |
...
- `project_expenses_labor`: `calculated_cost = hours / 8 * daily_cost`（从成本标准获取 daily_cost）
