...
**核心字段**：
- `id`: 项目ID（唯一标识）
- `code`: 项目编号（自动生成，格式：PROJ-YYYY-XXX，不在UI中显示）
- `name`: 项目名称（项目制）或主项目名称（计件制）
- `type`: 项目类型
- `manager`: 项目经理（姓名）
- `group`: 归属部门（Department枚举）
- `bizManager`: 商务经理（姓名）
- `clientDept`: 归属客户部（ClientDepartment枚举）
...
#### Framework Agreement（框架协议/主项目）

**说明**：计件制项目必须关联到一个框架协议（主项目）

**核心字段**：
- `id`: 框架协议ID
- `code`: 框架协议编号（自动生成，格式：FRAM-YYYY-XXX，不在UI中显示）
- `name`: 主项目名称
- `manager`: 项目经理
- `bizManager`: 商务经理
- `dept`: 归属部门
...
**业务规则**：
- 计件制项目创建时，必须选择或创建框架协议
- 选择框架协议后，自动带出项目经理、商务经理、归属部门、归属客户部等信息
- 框架协议字段修改不需要审批，但会同步到关联需求的相关字段（建议采用同步方式，即需求不单独存储主项目字段，而是实时读取框架协议的最新信息）
...
**计件制（PIECEWORK）**：
- 适用场景：基于框架协议的需求项，按件计费
- 必填字段：框架协议（主项目）、需求名称、计划开始/结束日期、阶段模板、业绩金额（补录模式下不填写）、预算（人力预算必填，差旅和外包可选，补录模式下不填写）
- 特殊规则：
  - 必须选择或创建框架协议，选择后自动带出项目经理、商务经理、归属部门、归属客户部
  - 框架协议信息可以手动修改（修改后同步到关联需求）
  - 必须选择阶段模板，选择后自动创建项目阶段实例（可编辑阶段名称和占比）
...
- 特殊规则：
  ...
  - **人力成本计算**：来源于人员的工时 × 等级单价（从 Teambition 同步工时数据，自动计算成本）
  - 不需要展示财务汇总信息
...
- 可执行操作：项目变更、标记阶段完成、录入支出、补录申请（计件制待补录项目）

**进行中（IN_PROGRESS）**：
- 触发条件：到达计划开始日期时，状态自动变为"进行中"
- 自动处理：到达计划开始日期时，系统自动将状态从"待启动"变为"进行中"，并自动设置实际开始日期为计划开始日期（或允许手动调整）
- 可执行操作：项目变更、标记阶段完成、发起设计确认、录入支出

**待确认（PENDING_CONFIRM）**：
- 触发条件：项目进度达到100%（所有阶段完成）或手动设置为"待确认"
- 可执行操作：项目变更、标记阶段完成、确认完成（状态变为"已确认"）、录入支出、补录申请（计件制待补录项目）

**已确认（CONFIRMED）**：
- 触发条件：项目确认完成
- 可执行操作：标记阶段完成（部分情况下会有售后、维护等工作）、发起归档、录入支出、补录申请（计件制待补录项目）
- 不可执行操作：项目变更、发起设计确认

**已归档（ARCHIVED）**：
- 触发条件：已确认的项目发起归档申请并审批通过
- 可执行操作：取消归档（不需要审批，直接取消，状态变为"已确认"）
- 不可执行操作：项目变更、标记阶段完成、发起设计确认、录入支出、补录申请
...
- 阶段调整验证：调整后所有阶段占比之和必须等于100%

**计件制变更**：
- 变更类型选择（必选）：
  - 主项目变更（`changeType = 'project'`）：变更框架协议相关信息（项目经理、归属部门、商务经理、归属客户部），不需要验证预算，变更后同步到关联需求
...
**外包支出**：
- 手动录入，必填字段：统计月份、外包事项、供应商、金额（必须 > 0）
...
- 如果存在预算但无支出：
  - 显示提示信息，列出未核对的预算项（差旅预算/外包预算）
  - 要求用户核对确认：是否确实没有支出，还是遗漏了支出记录
  - 用户确认后，才能提交归档申请

**归档申请流程**：
1. 点击"发起归档"按钮
2. 系统自动检查预算与支出情况
3. 如果有预算但无支出，显示核对提示，用户确认后继续
4. 填写归档说明（可选）
...
- `id`: 阶段项ID
- `name`: 阶段名称
- `percentage`: 占比（0-100，所有阶段占比之和必须等于100）

### 8.5.2 项目阶段（实例化）

**项目阶段字段**：
- `id`: 阶段ID
- `projectId`: 关联的项目ID（项目制）或需求编号（计件制）
- `stageTemplateItemId`: 关联的模板阶段ID（可选，用于追溯模板）
- `name`: 阶段名称（可编辑）
...
- **标记为完成**：勾选阶段为"完成"状态，阶段完成百分比自动为100%
...
- 回退后，阶段完成百分比重置为0（进行中）或保持当前值（可手动调整）
- 回退后重新计算项目进度
- 记录回退操作日志
...
- 所有阶段完成后（所有阶段完成百分比 = 100），进度自动为100%，可手动设置为"待确认"
...
- 可以随时标记阶段为"进行中"或"完成"
- 阶段进行中时，可以设置阶段完成百分比（0-100），用于体现工作周期较长阶段的进度变化
- 标记阶段完成时，阶段完成百分比自动为100%，可上传附件（可选）
- 系统自动记录完成时间和完成人
- 自动计算项目进度：`进度 = sum(阶段.percentage × 阶段完成百分比 / 100)`

**进度达到100%**：
- 所有阶段完成后（所有阶段完成百分比 = 100），进度自动为100%
- 可手动设置为"待确认"状态
...
1. **项目ID唯一性**：每个项目必须有唯一的项目ID
2. **框架协议关联**：计件制项目必须关联到框架协议
...
1. **已归档项目**：不能做变更、不能标记阶段完成、不能发起设计确认、不能录入支出、不能补录申请
2. **待补录标记的项目**（`isPendingEntry = true`）：不能做变更，但可以标记阶段完成（状态可以是"待启动"、"进行中"、"待确认"、"已确认"）、可以发起设计确认（状态为"进行中"或"待确认"时）、可以录入支出、可以补录申请
3. **已确认项目**：不能做变更，但可以标记阶段完成（因为部分情况下会有一些工作是要在设计确认后开展的，例如售后、维护等工作）、可以录入支出
...
5. **录入支出**：待启动/进行中/待确认/已确认状态都可以录入支出（因为工时填报和差旅外包支出的录入一般是滞后的）
...
**列表显示字段**：项目名称/需求名称、项目类型、项目经理、归属部门、商务经理、计划开始/结束日期、项目状态、进度、业绩金额、完成业绩、财务汇总（预算/支出/利润）、操作按钮
...
- 断点：xs(24列)、sm(12列)、md(12列)、lg(6列)、xl(6列)、xxl(6列)
- 一行最多4列录入组件（lg及以上）
...
3. 预算和支出管理支持人力、差旅、外包三种类型，人力支出通过系统内的任务和工时管理模块录入
