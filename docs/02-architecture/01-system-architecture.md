...
本系统采用**前后端一体化架构**（Full-Stack Framework），使用 Next.js 14 作为全栈框架，结合 **Supabase** 作为主要后端服务（数据库、认证），并采用**混合云方案**，使用阿里云服务进行应用部署和补充服务（文件存储、CDN 加速等）。

**架构特点**：

- **服务端渲染（SSR）**：提升首屏加载速度和 SEO
- **API 路由**：Next.js API Routes 处理服务端逻辑
...
- `components/ui/`：基础 UI 组件（Arco Design React）
...
- **Server Components**：减少客户端 JavaScript，提升性能
